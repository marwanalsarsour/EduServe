...
<main class="flex-grow-1 container my-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <?php if (!empty($data['notifications'])): ?>
            <a href="/supervisor/notifications/mark-read"
        ...
        <div class="list-group list-group-flush">
            ...
            <?php if (!empty($data['notifications'])): ?>
                ...
                <?php foreach ($data['notifications'] as $notif): ?>
                    ...
                    <?php
                        // (اختياري) تحديد أيقونة حسب نوع العنوان
                        $icon = 'bi-bell';

                        if (str_contains($notif['title'], 'تقرير')) {
                            $icon = 'bi-file-earmark-text';
                        } elseif (str_contains($notif['title'], 'فرصة')) {
                            $icon = 'bi-briefcase';
                        } elseif (str_contains($notif['title'], 'طلب')) {
                            $icon = 'bi-person-plus';
                        }
                    ?>

                    <div class="list-group-item list-group-item-action p-3 border-0 border-bottom
                        <?= !empty($notif['is_read']) ? '' : 'bg-white border-end border-primary border-4' ?>">
                        ...
                        <div class="d-flex w-100 justify-content-between align-items-center">
                            ...
                            <small class="text-muted small">
                                <?= htmlspecialchars($notif['time_ago']) ?>
                            </small>
