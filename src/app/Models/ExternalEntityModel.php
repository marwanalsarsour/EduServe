...
class ExternalEntityModel {
    ...
    public function deleteOpportunity($id) {
        try {
            // بدء عملية مجمعة لضمان حذف الطلبات المرتبطة أولاً وتفادي الـ Foreign Key Exception
            $this->db->beginTransaction();

            // 1. حذف كافة الطلبات المرتبطة بالفرصة من جدول OpportunityRequest
            $stmtRequests = $this->db->prepare("DELETE FROM OpportunityRequest WHERE opportunityID = :id");
            $stmtRequests->execute([':id' => $id]);

            // 2. حذف الفرصة نفسها من جدول Opportunity
            $stmtOpp = $this->db->prepare("DELETE FROM Opportunity WHERE opportunityID = :id");
            $result = $stmtOpp->execute([':id' => $id]);

            $this->db->commit();
            return $result;
        } catch (Exception $e) {
            // في حال حدوث أي مشكلة نلغي العملية للحفاظ على سلامة البيانات
    ...
    public function saveCertificate($data) {
        $stmt = $this->db->prepare("
            ...
            VALUES (:certificate_id, :issue_date, :is_approved, :student_id, :entity_id, :opportunity_id)
        ");
        ...
        return $result ? $generatedID : false;
    }
